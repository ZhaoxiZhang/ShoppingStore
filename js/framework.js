...
function fadeIn(i, elements, duration, callback){ if(i >= elements.length) typeof callback == 'function' && callback(); else elements.eq(i).fadeIn(duration,function(){ fadeIn(i+1, elements, duration, callback); }); }
function GoFullscreen(element) { if(element.requestFullscreen) { element.requestFullscreen(); } else if(element.mozRequestFullScreen) { element.mozRequestFullScreen(); } else if(element.webkitRequestFullscreen) { element.webkitRequestFullscreen(); } else if(element.msRequestFullscreen) { element.msRequestFullscreen(); } }
function ExitFullscreen() { if(document.exitFullscreen) { document.exitFullscreen(); } else if(document.mozCancelFullScreen) { document.mozCancelFullScreen(); } else if(document.webkitExitFullscreen) { document.webkitExitFullscreen(); } }
function toIntArray(value, delimiter) { var arr = []; var values = value.trim().split(delimiter); for (var i = 0; i < values.length; i++) arr.push(parseInt(values[i])); return arr; }
...
var dg = ['Zero','One','Two','Three','Four', 'Five','Six','Seven','Eight','Nine']; var tn = ['Ten','Eleven','Twelve','Thirteen', 'Fourteen','Fifteen','Sixteen', 'Seventeen','Eighteen','Nineteen']; var tw = ['Twenty','Thirty','Forty','Fifty', 'Sixty','Seventy','Eighty','Ninety']; function numToWord(s){s = s.toString(); s = s.replace(/[\, ]/g,''); if (s != parseFloat(s)) return 'not a number'; var x = s.indexOf('.'); if (x == -1) x = s.length; if (x > 15) return 'too big'; var n = s.split(''); var str = ''; var sk = 0; for (var i=0; i < x; i++) {if ((x-i)%3==2) {if (n[i] == '1') {str += tn[Number(n[i+1])] + ' '; i++; sk=1;} else if (n[i]!=0) {str += tw[n[i]-2] + ' ';sk=1;}} else if (n[i]!=0) {str += dg[n[i]] +' '; if ((x-i)%3==0) str += 'hundred ';sk=1;} if ((x-i)%3==1) {if (sk) str += th[(x-i-1)/3] + ' ';sk=0;}} if (x != s.length) {var y = s.length; str += 'point '; for (var i=x+1; i<y; i++) str += dg[n[i]] +' ';} return str.replace(/\s+/g,' ');}
//END FUNCTIONS
...
$(function() {
	...
	if (sidebarOpened)
		$(".sidebar li.selected").each(function() {
			$(this).find("ul").show().css("height", "auto").css("margin-left", "0").attr('style', function(i,s) { return (s||'') + 'display: block !important;' });
		});
		...
		if (!$("body").hasClass("sidebar-hover") && !sidebarOpened) {
			if (!sidebarOpened) {
				...
				if ($(this).parent().find("> ul").length > 0) {
					...
					$(".sidebar-item-info ul li ul .selected").parent().show().css("height", "auto").attr('style', function(i,s) { return (s||'') + 'display: block !important;' });
				}
				...
				$('.sidebar-item-info').stop(true, true).fadeIn({ duration: 100, queue: false }).animate({left: "55px"}, 100);
	...
	$(".sidebar-item-info").mouseleave(function() {
		if (!$("body").hasClass("sidebar-hover") && !sidebarOpened) {
			$('.sidebar-item-info').stop(true, true).fadeOut({ duration: 100, queue: false }).animate({left: "0"}, 100).css("bottom", "");//.fadeIn(50);
			...
			if ($(this).parent().find("> ul").length > 0) {
				if ($(this).parent().find("> ul").css("display") == "none") {
					...
					if ($(".sidebar").hasClass("no-animation")) {
						$(this).parent().find("> ul").show().css("height", "auto").attr('style', function(i,s) { return (s||'') + 'display: block !important;' });
						$(this).parent().find("ul").show();
					}
					else {
						$(this).parent().find("> ul").stop(true, true).fadeIn({ duration: 100, queue: false }).css("height", "auto").animate({marginLeft: "0%"}, 100).attr('style', function(i,s) { return (s||'') + 'display: block !important;' });
					}
					...
					if ($(".sidebar").hasClass("no-animation")) {
						...
						$(this).parent().find("> ul").stop(true, true).fadeOut({ duration: 100, queue: false }).animate({height: "0"}, 100, function() {
							$(this).parent().find("> ul").css("margin-left", "-50%");
						});
						$(this).removeClass("active");
						$(this).parent().find("> ul").removeClass("opened");
	...
	$(".content-container .black").click(function() {
		...
		$(".search-content").stop(true, true).fadeOut({ duration: 100, queue: false }).animate({top: (-h) + "px"}, 100);
	...
	$(".navbar-options .hover").click(function() { return false; });
...
function Resizing() {
	...
	if (width <= 700) {
		...
		var w = 10; $('.second-navbar .navbar-options > *').each(function() { w += $(this).outerWidth(); });
